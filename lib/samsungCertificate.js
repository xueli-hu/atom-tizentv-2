var samsungCertificate = (function() {
    ...
    var SamsungCertManager = require('@tizentv/webide-common-tizentv').SamsungCertManager;
        ...
        dialog.showMessageBox({
			...
			if (response == 0)
			{
                loginSamsungAccount(authorCreatePage).then((accessInfo) => {
                    ...
                    samsungCertMgr.createAuthorCert(profileName, {
                        name : authorInfo.authorName,
                        password : authorInfo.authorPassword,
                        country : authorInfo.authorCountry,
                        state : authorInfo.authorState,
                        city : authorInfo.authorCity,
                        organization : authorInfo.authorOrganization,
                        department : authorInfo.authorDepartment
                    }, accessInfo).then(async (message) => {
                        authorCreatePage.getView().loadingStop();
                        if (message == 'success') {
                            let authorProfile = {
                                authorCA: '',
                                authorCertPath: common.SamsungCertPath + sep + profileName + sep + 'author.p12',
                                authorPassword: authorInfo.authorPassword
                            };
                            ...
                            dialog.showMessageBox({
                                ...
                                detail: authorProfile.authorCertPath,
                                buttons: ['OK']
                            }, () => {
    ...
    var distributorGenerate = function(profileName, distributorInfo, distributorCert) {
        ...
        samsungCertMgr.createDistributorCert(profileName, distributorInfo, distributorCert.accessInfo).then(async (message) => {
            ...
            if (message == 'success') {
                ...
                if (profileMgr.isProfileExist(profileName) == true) { // Profile select case and author create case
                    await profileMgr.modifyProfile(profileName, 'distributor1', distributorProfile.key, distributorInfo.distributorPassword);
                }
    ...
    var selectDistributorCert = async function() {
        let profileName = profilePage.getView().getInfo();
        let distributorInfo = distributorSelectPage.getView().getInfo();

        if (profileMgr.isProfileExist(profileName) == true) { // Profile select case and author create case
            await profileMgr.modifyProfile(profileName, 'distributor1', distributorInfo.distributorFile, distributorInfo.distributorPassword);
        }
        else { // Author select case
            let authorSelectInfo = authorSelectPage.getView().getInfo();
            let authorProfile = {
                authorCA: '',
                authorCertPath: authorSelectInfo.authorFile,
                authorPassword: authorSelectInfo.authorPassword
            };
            let distributorProfileTemp = {
                distributorCA: '',
                distributorCertPath: distributorInfo.distributorFile,
                distributorPassword: distributorInfo.distributorPassword
            }
            await profileMgr.registerProfile(profileName, authorProfile, distributorProfileTemp);
        }
    ...
    return {
        handleCommand:async function() {
            try {
                samsungCertMgr = new SamsungCertManager(common.resourcePath);
                await samsungCertMgr.init();
                profileMgr = new ProfileManager(common.resourcePath);
                ...
                PageObject.addFinishListener(() => destroyAllPage());
                logger.error(moduleName, err);
                common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.ERROR, err);
            }
        }
    }
})();
