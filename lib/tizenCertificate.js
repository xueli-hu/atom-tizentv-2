var tizenCertificate = (function(){
	...
	return {
		handleCommand:async function() {
			...
			tizenCertificateViewControl.showView().then(async (certInfo) => {
				...
				let tizenAuthorProfile = null;
				let tizenDistributorProfile = null;
				...
				try {
					...
					if (certInfo.authorInfo.type == 'create') {
						tizenCertMgr.createCert({
							keyFileName: certInfo.authorInfo.info.authorFile,
							authorName: certInfo.authorInfo.info.authorName,
							authorPassword: certInfo.authorInfo.info.authorPassword,
							countryInfo: certInfo.authorInfo.info.authorCountry,
							stateInfo: certInfo.authorInfo.info.authorState,
							cityInfo: certInfo.authorInfo.info.authorCity,
							organizationInfo: certInfo.authorInfo.info.authorOrganization,
							departmentInfo: certInfo.authorInfo.info.authorDepartment,
							emailInfo: certInfo.authorInfo.info.authorEmail
						});
					...
					if (certInfo.distributorInfo.type == 'default') {
						tizenDistributorProfile = tizenCertMgr.getTizenDistributorProfile(certInfo.distributorInfo.info);
					}
					else if (certInfo.distributorInfo.type == 'select') {
						tizenDistributorProfile = {
							distributorCA: '',
					...
					await profileMgr.registerProfile(profileName, tizenAuthorProfile, tizenDistributorProfile);
					common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.INFO, 'Creating new profile successful.');
