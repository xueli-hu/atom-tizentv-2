var certificateManager = (function(){
    ...
    var TizenCertManager = require('@tizentv/webide-common-tizentv').TizenCertManager;
    ...
    var setActiveProfile = function(){
		...
		if(profileNames && profileNames.length > 0){
			...
			projectListView.showSelectList(activeChoices, selectTip).then(function (choice) {
				...
				if (setActiveProfileItem(activeName)) {
					common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.INFO, 'Set active profile '+ activeName + ' successful');
				} else {
					common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.ERROR, 'Set active profile '+ activeName + ' failed');
				}
    ...
    var removeProfileItem = function(name) {
		...
		if(fs.existsSync(common.profilePath)){
			...
			if (profileMgr.removeProfile(name)) {
				common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.INFO, 'Remove ' + name + ' profile successful');
			} else {
				common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.ERROR, 'Remove ' + name + ' profile failed');
			}
		}else{
	...
	var removeCertificateFiles = function(profileData, authorFile, distributorFile) {
		...
		if (isAuthorUsed == false) {
			if (authorFile.includes(common.AuthorPath)) { // Remove Tizen Ceritifcate
				if (fs.existsSync(authorFile)) {
					fs.unlinkSync(authorFile);
				}
			}
			else if (authorFile.includes(common.SamsungCertPath)) { // Remove Samsung Certificate
		...
		if (isDistributorUsed == false) {
			if (distributorFile.includes(common.SamsungCertPath)) { // Remove Samsung Certificate
    ...
    var setActiveProfileItem =function(name){
		if(fs.existsSync(common.profilePath)){
			let profileMgr = new ProfileManager(common.resourcePath);
			return profileMgr.setActivateProfile(name);
		}else{
			var waringMsg = 'The ' + common.profilePath + ' is not exist' ;
			logger.warning(moduleName, waringMsg);
			common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.WARNING, waringMsg);
    ...
    var getActiveProfileName = function(){
		...
		var activeName = '';
		if(fs.existsSync(common.profilePath)){
			logger.info(moduleName, 'Profile file path:' + common.profilePath);
			var data = fs.readFileSync(common.profilePath,'utf-8');
			//parse profiles.xml file to get author and distributor p12 certificate file 
			parseString(data,{ explicitArray : false}, function(err,result){
				var jsonData = JSON.stringify(result);
	...
	var changeCertificate = function(label, type, profilename){
		...
		projectListView.showInputBox(promptMsg1).then(function (certpath) {
			...
			if(fs.existsSync(certpath) && extension == '.p12'){
				projectListView.showInputBox('Please enter the certificate password you selected').then(async function(password){
					if(!password){
						...
						let profileMgr = new ProfileManager(common.resourcePath);
						if (await profileMgr.modifyProfile(profilename, type, certpath, password)) {
							common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.INFO, 'Change Certificate successful');
						} else {
							common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.WARNING, 'Change Certificate failed');
						}
					}
	...
	var removeDistributorCert = function( type, profilename, certPath){
		...
		projectListView.showSelectList(choices, certificateTip).then(async function (choice) {
			if (!choice) {
			...
			if(choice == 'Yes'){
				let profileMgr = new ProfileManager(common.resourcePath);
				if (await profileMgr.modifyProfile(profilename, type, '', '')) {
					common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.INFO, 'Remove Certificate successful');
				} else {
					common.showMsgOnWindow(common.ENUM_WINMSG_LEVEL.WARNING, 'Remove Certificate failed');
				}
			}else{
	...
	var detailCertificateInfo = async function(certPath, password){
		...
		var parseInfo = await tizenCertMgr.getCertificateInfo(certPath, password);
		if(parseInfo){
	...
	return {
        ...
        handleCommand:function() {
            ...
            if (fs.existsSync(common.profilePath)) {
                try {
                    fs.accessSync(common.profilePath, fs.constants.R_OK | fs.constants.W_OK);
                } catch (err) {
                    console.info('modify access permission!');
                    fs.chmodSync(common.profilePath, 0666);
                }
            }
